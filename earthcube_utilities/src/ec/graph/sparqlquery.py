...
{"g": self.repo}
"""
def queryWithSparql( template_name, parameters, endpoint):
    query = getFileFromResources(f"sparql/{template_name}.txt")
...
def getAGraph(  g, endpoint):
    query = getFileFromResources('sparql/get_triples_for_a_graph.txt')
    q_template = Template(query)
